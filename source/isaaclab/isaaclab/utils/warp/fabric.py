...
def compose_fabric_transformation_matrix_from_warp_arrays(
    ...
    fabric_matrices[fabric_index] = wp.mat44d(  # type: ignore[arg-type]
        wp.transpose(_compose_transformation_matrix(position, rotation, scale))  # type: ignore[arg-type]
    )


@wp.func
def _compose_transformation_matrix(position: Any, rotation: Any, scale: Any):  # -> wp.mat44f
    """Compose a 4x4 transformation matrix from position, rotation quaternion, and scale.

    Inverse of ``_decompose_transformation_matrix``.  Uses only Warp 1.x primitives
    (``quat_to_matrix``) so it works on Isaac Sim 4.5.
    """
    rot = wp.mat33f(wp.quat_to_matrix(rotation))  # type: ignore[arg-type]
    return wp.mat44f(

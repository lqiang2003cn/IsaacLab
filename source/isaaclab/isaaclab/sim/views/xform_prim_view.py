...
class _FabricHierarchyFallback:
    """Lightweight stand-in for ``usdrt.hierarchy.IFabricHierarchy`` on Isaac Sim < 5.0.

    Calls ``Rt.Xformable.SetWorldXformFromUsd()`` on every tracked prim to ensure
    world matrices are up-to-date.  This is slower than the native batch API but
    functionally equivalent for the small prim counts typical in IsaacLab views.
    """

    def __init__(self, view: XformPrimView):
        self._view = view

    def update_world_xforms(self) -> None:
        import usdrt
        ...
        for path in self._view.prim_paths:
            ...
            Rt.Xformable(rt_prim).SetWorldXformFromUsd()
...
class XformPrimView:
    ...
    def _initialize_fabric(self) -> None:
        ...
        # usdrt.hierarchy is only available in Isaac Sim >= 5.0; fall back to a lightweight wrapper
        # that calls SetWorldXformFromUsd() per-prim when the API is missing.
        if hasattr(usdrt, "hierarchy"):
            ...
            self._fabric_hierarchy = _FabricHierarchyFallback(self)
        self._fabric_hierarchy.update_world_xforms()
